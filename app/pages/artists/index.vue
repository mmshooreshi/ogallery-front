...
const { data } = await useAsyncData('artists', () => queryCollection('artists').all())
...
const items = computed(() => {
  ...
  list.sort((a:any,b:any)=>((a?.sortIndex??1e9)-(b?.sortIndex??1e9)) || String(a?.name||'').localeCompare(b?.name||''))
  const byQ = q.value ? list.filter((it:any)=>String(it?.name||'').toLowerCase().includes(q.value)) : list
  return letter.value ? byQ.filter((it:any)=>String(it?.name||'').toUpperCase().startsWith(letter.value)) : byQ
})

const initials = computed(() => {
  const set = new Set((data.value ?? []).map((it:any)=>String(it?.name||'').charAt(0).toUpperCase()).filter(Boolean))
...
<template>
  <section>
    ...
    <nav v-if="initials.length" class="flex flex-wrap gap-2 mb-6 text-sm">
      <NuxtLink class="px-2 py-1 border rounded" :class="{ 'bg-black text-white': !letter }"
                :to="{ query: { ...$route.query, letter: undefined } }">All</NuxtLink>
      <NuxtLink v-for="ch in initials" :key="ch" class="px-2 py-1 border rounded"
                :class="{ 'bg-black text-white': letter === ch }"
                :to="{ query: { ...$route.query, letter: ch } }">{{ ch }}</NuxtLink>
    </nav>

    <ul v-if="items.length" class="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      <li v-for="doc in items" :key="doc.slug" class="flex items-center gap-3">
        <NuxtLink :to="`/artists/${doc.slug}`" class="group flex items-center gap-3">
          <NuxtImg v-if="doc.image" :src="doc.image" width="64" height="64" class="rounded-full object-cover" :alt="doc.name" />
