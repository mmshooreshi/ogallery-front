<!-- app/pages/exhibitions/indexOriginal.vue -->
<script setup lang="ts">
import { useLocalCache } from '~/composables/useLocalCache'

type ExhibitionListItem = { id: number; slug: string; name: string; coverImage: string | null }

const { locale } = useI18n()
const route = useRoute()
const key  = computed(() => `exhibitions:${locale.value}`)

const { data: exhibitions, pending, error } = useLocalCache<ExhibitionListItem[]>(
  () => key.value,
  () => $fetch<any>('/_q/exhibitions', { query: { locale: locale.value } }),
...
const items = computed(() => {
  const base = (exhibitions.value ?? []).slice().sort((a, b) => a.name.localeCompare(b.name))
...
const initials = computed(() =>
  Array.from(new Set((exhibitions.value ?? []).map(it => it.name[0]?.toUpperCase()).filter(Boolean))).sort()
)
    ...
    <ClientOnly>
      ...
      <ul v-if="items.length" class="grid gap-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 list-none">
        <li v-for="doc in items" :key="doc.slug">
          <NuxtLink :to="`/exhibitions/${doc.slug}`" class="no-underline group flex items-center gap-2">
      ...
      <p v-else class="opacity-70">No exhibitions found.</p>
