...
export type ScraperConfig = {
  ...
  selectors: {
    ...
    works: {
      ...
      deepScrape?: {
        enabled: boolean;
        images: { selector: string; attr: string }; // e.g. .carousel-item img
        caption?: string; // Selector for caption on child page
        status?: string; // Selector for status on child page
      };
    };
...
export type ScrapedWork = {
  ...
  status?: string | null;
  extraImages?: string[]; // For gallery details
}
...
export class OGalleryScraper {
  ...
  private log(level: ScrapeLog["level"], message: string, context?: any) {
    ...
    if (level === "error" || level === "warn") {
      console.log(
        `[Scraper][${this.config.type}][${level}] ${message}`,
        context
      );
    }
  }
  ...
  async fetchList(): Promise<ScrapedListItem[]> {
    ...
    try {
      ...
      this.log(
        "info",
        `[LIST] Selector "${this.config.selectors.listItems}" found ${listElements.length} elements.`
      );
      ...
      listElements.each((_i, el) => {
        ...
        const expectedSegment = this.config.paths.list
          .split("/")
          .filter(Boolean)[1]
          .toLowerCase();
        ...
        if (
          parts.length === 3 &&
          parts[0] === "en" &&
          parts[1].toLowerCase() === expectedSegment
        ) {
          const slug = parts[2];
          if (slug && !seen.has(slug)) {
            ...
            const sourceUrlEn =
              this.abs(this.config.paths.detail(slug, "EN" as Locale)) || "";
            const sourceUrlFa =
              this.abs(this.config.paths.detail(slug, "FA" as Locale)) || "";
            ...
            this.log(
              "info",
              `[LIST] Skipped href/text: ${href} | ${text} (Not a detail page or duplicate)`
            );
          }
        }
      });
      ...
      this.log(
        "info",
        `[LIST] Successfully processed and found ${items.length} final items.`
      );
      return items;
    } catch (e: any) {
      this.log(
        "error",
        `[LIST] Failed to fetch list or process HTML.`,
        e.message
      );
      return [];
    }
    ...
    try {
      ...
      if (this.config.selectors.customProps) {
        for (const [key, selector] of Object.entries(
          this.config.selectors.customProps
        )) {
      ...
      if (publishDateSel) {
        ...
        const publishDate = $(publishDateSel)
        ...
        let pdfLink = container
          ...
          .filter((_i, el) => {
            ...
            if (cvConfig.keywords && cvConfig.keywords.length > 0) {
              return cvConfig.keywords.some(
                (k) => href.includes(k) || text.includes(k)
              );
            }
            return true;
          })
      ...
      if (this.config.selectors.metadata?.container) {
        const lines = this.extractEventLinesForNews(
          $,
          this.config.selectors.metadata.container
        );
        ...
        if (!props.dateString) {
          const dateLine =
            lines.find((x) => /\d{4}/.test(x) && /-/.test(x)) || null;
          if (dateLine) {
  ...
  private parseDateRange(raw: string): {
  ...
  private extractArtistLinksForNews(
    $: any
  ): Array<{ slug: string; name: string; href: string }> {
    ...
    $("a[href]").each((_i: number, el: any) => {
      ...
      let u: URL;
      try {
        u = new URL(href, this.config.baseUrl);
      } catch {
        return;
      }
  ...
  private async scrapeListMetadata(
    slug: string
  ...
  async scrapeDetail(slug: string): Promise<ScrapeResult> {
    ...
    const [enLocale, faLocaleRaw, media, metadata, listExtras] =
      await Promise.all([
    ...
    if (!enLocale.bodyHtml && enLocale.sections?.length) {
      enLocale.bodyHtml = enLocale.sections
        .map((s) => s.blocks.map((b) => b.html).join("\n"))
        .join("\n");
      enLocale.bodyText = enLocale.sections
        .map((s) => s.blocks.map((b) => b.text).join("\n"))
        .join("\n");
    }

    if (!faLocaleRaw.bodyHtml && faLocaleRaw.sections?.length) {
      faLocaleRaw.bodyHtml = faLocaleRaw.sections
        .map((s) => s.blocks.map((b) => b.html).join("\n"))
        .join("\n");
      faLocaleRaw.bodyText = faLocaleRaw.sections
        .map((s) => s.blocks.map((b) => b.text).join("\n"))
        .join("\n");
    }
    ...
    try {
      ...
      if (bodySel.preferredId && $(bodySel.preferredId).length) {
        bodyRow = $(bodySel.preferredId)
          .nextAll(".row")
          .filter(
            (_i, el) => $(el).find(bodySel.headingTags.join(",")).length > 0
          )
          .first();

        if (bodyRow.length) {
          this.log("info", `[${locale}] Found body row via preferred ID`);
        }
      }
      ...
      if (!bodyRow || !bodyRow.length) {
        bodyRow = $(".row")
          .filter((_i, el) => {
            const headers = $(el).find(bodySel.headingTags.join(","));
            ...
            return headers.get().some((h) => {
              ...
              if (match) {
                this.log("info", `[${locale}] Found section header: "${raw}"`);
              }
      ...
      // [CHANGED] Strategy C — Direct Content Wrapper Match (For Viewing Rooms)
      if (!bodyRow || !bodyRow.length) {
         const directWrapper = $(bodySel.contentWrapper).filter((_i, el) => {
            // It must have paragraphs and NOT be an artwork container
            return $(el).find('p').length > 0 && $(el).find('.artwork-viewingroom').length === 0;
         }).first();

         if (directWrapper.length) {
             bodyRow = directWrapper.closest('.row'); 
             this.log('info', `[${locale}] Found body via direct content wrapper.`);
         }
      }

      if (!bodyRow || !bodyRow.length) {
        this.log("warn", `[${locale}] No body row found`, { keywords });
      ...
      bodyText = contentCol.text().replace(/\s+/g, " ").trim() || null;
      ...
      headers.each((_i, headerEl) => {
        ...
        $section("p").each((_i, p) => {
          ...
          const isInfo =
            /MONDAY|TUESDAY|FRIDAY|SATURDAY|PM|AM|TELL|TEL|PHONE|خیابان|دوشنبه|جمعه/i.test(
              text
            );
          blocks.push({
      ...
      if (cvSel) {
        const linkContainer = bodyRow.find(cvSel.rowSelector).first();
      ...
      if (portfolioSel) {
        const portfolioLink = $('a[href$=".pdf"]')
          .filter((_i, el) => {
            const href = ($(el).attr("href") || "").toLowerCase();
            return portfolioSel.keywords.some((k) => href.includes(k));
          })
          .first();
  ...
  private async scrapeMedia(
    slug: string
  ): Promise<{ works: ScrapedWork[]; installations: { full: string }[] }> {
    ...
    // 1. Build Lookup Maps for FA works
    const faByFull = new Map<string, ScrapedWork>();
    const faByThumb = new Map<string, ScrapedWork>();

    for (const w of faWorksRes.works) {
        if (w.full) faByFull.set(w.full, w);
        if (w.thumb) faByThumb.set(w.thumb, w);
    }

    // 2. Merge EN with FA (Try Full URL first, then Thumbnail)
    const mergedWorks: ScrapedWork[] = enWorksRes.works.map(en => {
        let fa = faByFull.get(en.full);
        
        // Fallback: Match by thumbnail (Fixes Viewing Room URL differences)
        if (!fa && en.thumb) {
            fa = faByThumb.get(en.thumb);
        }

        return {
            full: en.full,
            thumb: en.thumb,
            captionEn: en.captionEn || null,
            captionFa: fa?.captionFa || null, 
            status: en.status || fa?.status || null,
            extraImages: en.extraImages // Keep extra images from EN scan
        };
    });

    // 3. Add FA-only works
    for (const fa of faWorksRes.works) {
        const alreadyExists = mergedWorks.some(m => 
            m.full === fa.full || (m.thumb && m.thumb === fa.thumb)
        );
    ...
    if (installationSel?.selector && url) {
      this.log(
        "info",
        `[EN] Attempting to scrape installations using selector: ${installationSel.selector}`
      );
      try {
        const html = await $fetch<string>(url);
        const $ = load(html);
        ...
        $(installationSel.selector).each((_i, el) => {
          const src = this.abs(
            $(el).attr("src") || $(el).attr("data-src") || null
          );
      ...
      this.log(
        "info",
        `Installation scraping skipped (no configuration or invalid URL).`
      );
    }
  ...
  //   locale: Locale
  // ): Promise<{
  //   works: { full: string; thumb: string | null; caption: string | null }[];
  // }> {
  //   const prefix = locale === "EN" ? "EN" : "FA";
  //   const url = this.abs(this.config.paths.detail(slug, prefix));
  ...
  //           $(element).attr(workSel.captionAttr) || null;
  ...
  //           fullUrl = this.abs(
  //             $(element).attr("src") || $(element).attr("data-src") || null
  //           );
  //           thumbUrl = fullUrl;
...
// [CHANGED] Rewritten to support Child Page scraping
  private async scrapeWorksForLocale(
    slug: string,
    locale: Locale,
  ): Promise<{ works: ScrapedWork[] }> {
      const prefix = locale === 'EN' ? 'EN' : 'FA'
      const url = this.abs(this.config.paths.detail(slug, prefix));
      ...
      try {
          ...
          const workSel = this.config.selectors.works;
          const doDeepScrape = workSel.deepScrape?.enabled ?? false;

          if (workSel) {
              const tasks: Promise<ScrapedWork | null>[] = [];
              ...
              $(workSel.container).each((_i, el) => {
                  ...
                  // 1. Try to find Status/Caption in Parent Grid
                  const parent = $(element).closest('figure, .exhibition-card, div');
                  const statusEl = parent.find('.artwork-viewingroom-status');
                  if (statusEl.length) status = statusEl.text().trim();
                  
                  if (!caption) {
                      caption = parent.find('.artwork-viewingroom-caption .caption, .caption').text().trim() || null;
                  }

                  // 2. Extract Link/Thumb
                  if (element.tagName === 'a') {
                      linkUrl = this.abs($(element).attr('href') || null);
                      const img = $(element).find('img').first();
                      thumbUrl = this.abs(img.attr('src') || img.attr('data-src') || null);
                      if (!caption) caption = img.attr('alt') || null;
                  } else if (element.tagName === 'img') {
                      linkUrl = this.abs($(element).attr('src') || $(element).attr('data-src') || null);
                      thumbUrl = linkUrl;
                      if (!caption) caption = $(element).attr('alt') || null;
                  }
                  ...
                  // 3. If Deep Scrape NOT enabled, return basic info
                  if (!doDeepScrape) {
                      works.push({
                          full: linkUrl,
                          thumb: thumbUrl,
                          captionEn: locale === 'EN' ? caption : null,
                          captionFa: locale === 'FA' ? caption : null,
                          status
                      });
                      return;
                  }
                  ...
                  tasks.push((async () => {
                      try {
                          const childHtml = await $fetch<string>(linkUrl!);
                          const $child = load(childHtml);
                          const ds = workSel.deepScrape!;

                          // A. Scrape Images (Carousel)
                          const childImages: string[] = [];
                          $child(ds.images.selector).each((_, imgEl) => {
                              const src = this.abs($child(imgEl).attr(ds.images.attr) || $child(imgEl).attr('src') || null);
                              if (src && !childImages.includes(src)) childImages.push(src);
                          });

                          if (childImages.length === 0) {
                              if (linkUrl!.match(/\.(jpg|jpeg|png|webp)$/i)) childImages.push(linkUrl!);
                              else if (thumbUrl) childImages.push(thumbUrl);
                          }

                          // B. Overwrite Caption/Status from Child
                          if (ds.caption) {
                              const childCap = $child(ds.caption).first().text().trim();
                              if (childCap) caption = childCap;
                          }
                          if (ds.status) {
                              const childStat = $child(ds.status).first().text().trim();
                              if (childStat) status = childStat;
                          }

                          const [mainImg, ...rest] = childImages;

                          return {
                              full: mainImg || linkUrl!,
                              thumb: thumbUrl || mainImg || linkUrl!,
                              captionEn: locale === 'EN' ? caption : null,
                              captionFa: locale === 'FA' ? caption : null,
                              status,
                              extraImages: rest
              ...
              if (doDeepScrape && tasks.length > 0) {
                  this.log('info', `[${locale}] Deep scraping ${tasks.length} works...`);
                  const results = await Promise.all(tasks);
                  results.forEach(w => { if (w) works.push(w); });
              }
          }
          return { works };
      } catch (e) {
