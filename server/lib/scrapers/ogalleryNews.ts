// server/lib/scrapers/ogalleryNews.ts
...
export type ScrapedNewsItemListItem = {
  slug: string
  nameEn: string
  urlEn: string
  urlFa: string
...
export type ScrapedNewsItemLocale = {
  slug: string
  locale: Locale
  title: string | null
  bioHtml: string | null
...
export type ScrapedNewsItemRich = {
  slug: string
  sourceUrlEn: string
  sourceUrlFa: string
  locales: ScrapedNewsItemLocale[]
  works: ScrapedWork[]
  installations: ScrapedInstallation[]
}

/**
 * Fast: just fetch /en/news and return list of news.
 */
export async function fetchEnNewsItemsLinks(): Promise<ScrapedNewsItemListItem[]> {
  const res = await fetch(`${BASE}/en/news`)
  if (!res.ok) {
    throw new Error(`Failed to fetch /en/news: ${res.status} ${res.statusText}`)
  }
  ...
  const news: ScrapedNewsItemListItem[] = []
  ...
  $('a[href]').each((_i, el) => {
    ...
    // match /en/news/{slug}
    if (parts.length === 3 && parts[0] === 'en' && parts[1] === 'news') {
      const slug = parts[2]
      if (!slug) return
      ...
      news.push({
        slug,
        nameEn: text,
        urlEn: `${BASE}/en/news/${slug}`,
        urlFa: `${BASE}/fa/news/${slug}`,
      })
    }
  })
 ...
 */
async function scrapeNewsItemPage(slug: string, locale: Locale): Promise<ScrapedNewsItemLocale> {
  const prefix = locale === 'EN' ? 'en' : 'fa'
  const url = `${BASE}/${prefix}/news/${slug}`
 ...
 */
export async function scrapeNewsItemRich(slug: string): Promise<ScrapedNewsItemRich> {
  const [enLocale, faLocaleRaw, media] = await Promise.all([
    scrapeNewsItemPage(slug, 'EN'),
    scrapeNewsItemPage(slug, 'FA'),
    scrapeWorksAndInstallations(slug),
  ])

  const faLocale: ScrapedNewsItemLocale = {
    ...faLocaleRaw,
  ...
  return {
    slug,
    sourceUrlEn: `${BASE}/en/news/${slug}`,
    sourceUrlFa: `${BASE}/fa/news/${slug}`,
