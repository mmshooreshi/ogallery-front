// server/lib/scrapers/ogalleryExhibitions.ts
...
export type ScrapedExhibitionListItem = {
  slug: string
  nameEn: string
  urlEn: string
  urlFa: string
...
export type ScrapedExhibitionLocale = {
  slug: string
  locale: Locale
  title: string | null
  bioHtml: string | null
...
export type ScrapedExhibitionRich = {
  slug: string
  sourceUrlEn: string
  sourceUrlFa: string
  locales: ScrapedExhibitionLocale[]
  works: ScrapedWork[]
  installations: ScrapedInstallation[]
}

/**
 * Fast: just fetch /en/exhibitions and return list of exhibitions.
 */
export async function fetchEnExhibitionLinks(): Promise<ScrapedExhibitionListItem[]> {
  const res = await fetch(`${BASE}/en/exhibitions`)
  if (!res.ok) {
    throw new Error(`Failed to fetch /en/exhibitions: ${res.status} ${res.statusText}`)
  }
  ...
  const exhibitions: ScrapedExhibitionListItem[] = []
  ...
  $('a[href]').each((_i, el) => {
    ...
    // match /en/exhibitions/{slug}
    if (parts.length === 3 && parts[0] === 'en' && parts[1] === 'exhibitions') {
      const slug = parts[2]
      if (!slug) return
      ...
      exhibitions.push({
        slug,
        nameEn: text,
        urlEn: `${BASE}/en/exhibitions/${slug}`,
        urlFa: `${BASE}/fa/exhibitions/${slug}`,
      })
    }
  })
...
/**
 * Scrape a single exhibition page for a given locale (structure-based, EN + FA).
 */
async function scrapeExhibitionPage(slug: string, locale: Locale): Promise<ScrapedExhibitionLocale> {
  const prefix = locale === 'EN' ? 'en' : 'fa'
  const url = `${BASE}/${prefix}/exhibitions/${slug}`
...
async function scrapeWorksAndInstallations(slug: string): Promise<{
  ...
  const url = `${BASE}/en/exhibitions/${slug}`
  const res = await fetch(url)
  if (!res.ok) {
...
/**
 * Full rich scrape for ONE exhibition, with EN+FA + works + installations.
 */
export async function scrapeExhibitionRich(slug: string): Promise<ScrapedExhibitionRich> {
  const [enLocale, faLocaleRaw, media] = await Promise.all([
    scrapeExhibitionPage(slug, 'EN'),
    scrapeExhibitionPage(slug, 'FA'),
    scrapeWorksAndInstallations(slug),
  ])

  const faLocale: ScrapedExhibitionLocale = {
    ...faLocaleRaw,
  ...
  return {
    slug,
    sourceUrlEn: `${BASE}/en/exhibitions/${slug}`,
    sourceUrlFa: `${BASE}/fa/exhibitions/${slug}`,
